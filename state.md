...
## Update — 2 Aug 2026
- **Phase 1 Slice 2c (Item photos) BUILT + VERIFIED locally — migration LIVE on remote, code NOT yet pushed/deployed.** One photo per menu item: pick in the item form → local preview → **deferred upload on Save** (upsert item, then POST the file; cancelling a new item leaves no orphaned file) → thumbnail on the customer card + full-width hero in the item sheet + thumbnail on the dashboard row. Remove clears `photo_url` and deletes the stored object.
...
- **NEXT ACTION for Lloyd:** `git push origin master` + Vercel CLI deploy to ship 2c to ordernook.uk. The storage migration is already live, so deploy is code-only. (Push/deploy left to you.)
