...
- **Phase B — source maps PENDING Lloyd:** stack traces are minified until `SENTRY_AUTH_TOKEN` is added to the Vercel build env (Sentry → Settings → Auth Tokens, or reuse BookOnTheMap's; then `vercel env add SENTRY_AUTH_TOKEN production` + redeploy). Build logs confirm the wrapper is active and just skips source-map upload without the token. Specs: `sentry-integration.md` (+ `-plan.md`).
